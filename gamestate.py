...
class Gamestate:
  ...
  def blank_ldiagonal(self):
    """
    Returns True if all the cells on the long diagonal of the board are blank.
    False otherwise.
    """
    for i in range(self.size):
  ...
  def blank_sdiagonal(self):
    """
    Returns True if all the cells on the short diagonal of the board are blank.
    False otherwise.
    """
    for i in range(self.size):
  ...
  def reachable(self, colors, stopset, start):
    """
    Returns a set containing the cells reachable from start by going through 
    cells of a color in colors and without crossing cells in the stopset.
    """
    seen = set([start])
    queue = deque([start])
    while len(queue) > 0:
