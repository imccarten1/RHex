...
class Node:
  ...
  def value(self, explore):
    ...
    "explore" specifies how much the value should favor nodes that have yet 
    to be thoroughly explored versus nodes that seem to have a high win rate. 
...
class Mctsagent:
  ...
  def select_node(self):
    ...
    while(len(node.children) !=0 ):
      ...
      max_nodes = [n for n in node.children 
  ...
  def roll_out(self, state):
    """
    Simulate an entirely random game from the passed state and return the
    winning player.
    """
    moves = state.moves()
  ...
  def backup(self, node, turn, outcome):
    """
    Update the node statistics on the path from the passed node to root to
    reflect the outcome of a randomly simulated playout.
    """
